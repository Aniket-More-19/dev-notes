...
async def get_all_notes():
    ...
    response = note_service.get_all_notes()
    return response
...
async def create_note(note_request: NoteRequest):
    note_service = NoteService()
    response = note_service.create_note(note_request)
    return response
...
async def update_note(note_id: int, updatedNnote: NoteRequest):
    note_service = NoteService()
    response = note_service.update_note(note_id, updatedNnote)
    return response
    
# delete note by id
@router.delete("/notes/deleteNote/{id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_note(note_id: int):
    note_service = NoteService()
    response = note_service.update_note(note_id)
    return response
